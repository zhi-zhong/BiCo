function MatingPool = MatingSelection2(Population,ArcPop,N)
...
%      Temp1     = sort(Cosine1,2,'descend');
    ...
    PopObj2 = (ArcPop.objs-repmat(max(ArcPop.objs,[],1),length(ArcPop.objs),1))./(repmat(max(ArcPop.objs),length(ArcPop.objs),1)-repmat(Zmin,length(ArcPop.objs),1)+1e-10)+1e-10;
   ...
   % Angle1 = Rank(1:N);
    ...
    while length(MatingPool)< N  
       ...
       % if rand < 0.5
      if CV1(Index1(i+1))< CV1(Index1(i+2))     
          MatingPool = [MatingPool,Population(Index1(i+1))];
      else
          MatingPool = [MatingPool,Population(Index1(i+2))];
      end
      
       if Angle2(Index2(i+1))< Angle2(Index2(i+2))     
          MatingPool = [MatingPool,Population(Index2(i+1))];
      else
          MatingPool = [MatingPool,Population(Index2(i+2))];
      end
